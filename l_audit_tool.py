...
def validate_sasl_mechanisms(supported_sasl_mechanisms):
	...
	for item in supported_sasl_mechanisms:
		match item:
			case "ANONYMOUS":
				print(f"\t{YELLOW}* {item} (COMMON){END} - The anonymous mechanism grants access to information by anyone.  For this reason it should be disabled by default so the administrator can make an explicit decision to enable it. (https://www.ietf.org/rfc/rfc2245.txt)")
			case "CRAM-MD5":
				print(f"\t{YELLOW}* {item} (LIMITED){END} - Even with the use of CRAM, users are still vulnerable to active attacks.  An example of an increasingly common active attack is 'TCP Session Hijacking' as described in CERT Advisory CA-95:01 [CERT95]. (https://www.ietf.org/rfc/rfc2195.txt)")
			case "DIGEST-MD5":
				print(f"\t{YELLOW}* {item} (OBSOLETE){END} - Digest Authentication does not provide a strong authentication mechanism, when compared to public key based mechanisms, for example. (https://www.ietf.org/rfc/rfc2831.txt)")
			case "PLAIN":
				print(f"\t{YELLOW}* {item} (COMMON){END} - By default, implementations SHOULD NOT advertise and SHOULD NOT make use of the PLAIN mechanism unless adequate data security services are in place. (https://www.rfc-editor.org/rfc/rfc4616.html)")
			case "LOGIN":
				print(f"\t{YELLOW}* {item} (OBSOLETE){END} - The LOGIN mechanism MUST NOT be advertised or used in any configuration that prohibits the PLAIN mechanism or a plaintext LOGIN (or USER/PASS) command that sends passwords in the clear. (https://datatracker.ietf.org/doc/html/draft-murchison-sasl-login-00)")
			case "EXTERNAL":
				print(f"\t{GREEN}* {item} (COMMON){END} - https://www.rfc-editor.org/rfc/rfc4422.html")
			case "GSSAPI":
				print(f"\t{GREEN}* {item} (COMMON){END} - https://www.rfc-editor.org/rfc/rfc4752.html")
			case "GSS-SPNEGO":
...
| | |______|  _  | | | | | | | | |   | |     | | | | | || | | | |    
| |____    | | | | |_| | |/ / _| |_  | |     | | \ \_/ /\ \_/ / |____
\_____/    \_| |_/\___/|___/  \___/  \_/     \_/  \___/  \___/\_____/v1.0
By Erick Rodríguez                                                                                                                                                                                          	
{END}
"""
system("clear")
...
parser.add_argument("-o", "--output", help = "Output to a text file")
args = parser.parse_args()

print(f"IP Address: {args.ip}\n")

print(f"{CYAN}Validating 'Anonymous Bind' through port 389{END}\n")
validate_anonymous_authentication(args.ip, 389)

print(f"\n{CYAN}Validating 'Anonymous Bind' through port 636{END}\n")
